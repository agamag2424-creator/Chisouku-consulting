...
import { verifyAppsScriptSuccess } from "@/app/api/_lib/apps-script";
...
export async function POST(request: Request) {
  ...
  if (!appScriptUrl.includes("/exec")) {
    return NextResponse.json(
      {
        error:
          "Insights waitlist endpoint looks invalid. Use an Apps Script Web App URL ending with /exec.",
      },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    const appScriptResult = await verifyAppsScriptSuccess(forwardResponse);
    if (!appScriptResult.success) {
      return NextResponse.json(
        {
          error: `Failed to save email. ${appScriptResult.error}`,
        },
        { status: 502 },
      );
    }
