...
export async function saveToAppsScript(
  ...
  try {
    ...
    return {
      success: false,
      error: "Failed to save nomination. Apps Script did not confirm the write.",
    };
  }
  ...
  return {
    success: false,
    error: `Failed to save nomination. Apps Script did not confirm the write.${upstreamError}`,
  };
}
